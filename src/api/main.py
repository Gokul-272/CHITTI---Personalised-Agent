...
async def lifespan(app: FastAPI):
    ...
    print(f"Ollama reachable: {ollama_alive()} (model: {settings.OLLAMA_MODEL})")
    print(f"Postgres reachable (readonly role): {db_reachable()}")
    yield


app = FastAPI(title="CHITTI - Personal Assistant Robot API", version="1.0.0", lifespan=lifespan)
