"""
src/ui/streamlit_app.py - Streamlit chat interface for CHITTI personal assistant.
...
same as src/ui/cli.py. Switch between RAG mode (CHITTI replies from his knowledge base)
and Agentic mode (CHITTI can call tools and take actions), and inspect exactly what
happened under the hood: retrieved sources in RAG mode, full Thought/Action/Observation
trace in Agentic mode.
"""
...
st.set_page_config(page_title="CHITTI", page_icon="🤖", layout="centered")
...
with st.sidebar:
    ...
    st.write(("✅" if kb_ok else "⚠️") + " CHITTI's memory " + ("loaded" if kb_ok else "not built yet"))
    ...
    st.write(("✅" if ollama_ok else "⚠️") + " Language model " + ("online" if ollama_ok else "offline"))
...
# ---------- main chat area ----------
st.header(f"🤖 CHITTI — {st.session_state.mode} mode")
if st.session_state.mode == "RAG":
    st.caption("CHITTI answers from his knowledge base. Precise. Factual. 100% accurate, Boss.")
else:
    st.caption("CHITTI is in full agentic mode. He can call tools, query the database, and take actions. Standing by, Boss.")


def render_trace(trace):
    with st.expander("See CHITTI's reasoning (Thought → Action → Observation)"):
...
query = st.chat_input("Ask CHITTI anything, Boss...")
...
if query:
    ...
    with st.chat_message("assistant"):
        if not api_ok:
            ...
            with st.spinner("CHITTI processing request, Boss..."):
                ...
                try:
                    ...
                    st.error(f"Request to CHITTI's API failed: {e}")
                    data = None
